...
class DEEExampleLoader(object):
    ...
    def rearrange_sent_info(self, detail_align_info):
        ...
        doc_text = ''.join(raw_sents)  # 将sentence转换为document
        ...
        # 因为句子长度是受限的，只能处理128个字符的句子。
        # 所以先尽可能的根据标点符合对句子进行划分，但在划分的过程中不能把已经存在的实体截断，需要权衡处理下
        short_sents = []
        ...
        # 将所有可能的短句子尽可能的合并为一个低于128个字符的长句子。
        comp_sents = ['']
        ...
        for raw_drange in raw_dranges:
            ...
            raw_char_base = raw_char_bases[raw_sid]  # 在Document中的句子索引
    ...
    def get_example_list(self, data_list):
        total_dee_examples = []
        for guid, detail_info in data_list:
            dee_example = self.convert_dict_to_example(guid, detail_info, only_inference=True)
    ...
    def __call__(self, dataset_json_path):
        ...
        for guid, detail_align_info in annguid_aligninfo_list:
            dee_example = self.convert_dict_to_example(guid, detail_align_info)
            total_dee_examples.append(dee_example)
...
class DEEFeature(object):
    ...
    def generate_dag_info_for(self, pred_span_token_tup_list, return_miss=False):
        ...
        # 当实际的span没有被预测出来时，保留实际span在文章中出现过的句子ID
    ...
    def build_key_event_sent_info(self):
        ...
        for key_sent_idx_set, event_label, event_arg_idxs_objs in zip(
            ...
            if event_label == 0:  # 文章中不存在该类型的事件
                assert event_arg_idxs_objs is None
            else:
                # 该事件类型下多个事件表示
                for event_arg_idxs_obj in event_arg_idxs_objs:
                    sent_idx_cands = []
                    for span_idx in event_arg_idxs_obj:  # event_arg_idxs_obj 是一个元组
                        if span_idx is None:
                            continue
                        # 该ID对应的 mention 在文中出现的所有位置
                    ...
                    # 对所有出现过 mention 的句子进行统计
                    sent_idx_cnter = Counter(sent_idx_cands)
                    # 选择出现不同 mention 个数最多的句子作为关键句
                    key_sent_idx = sent_idx_cnter.most_common()[0][0]
                    # 找到一个包含span_idx的核心句子，相同事件类型下，核心事件句需要去重
    ...
    def convert_example_to_feature(self, ex_idx, dee_example, log_flag=False):
        ...
        for sent_idx, sent_text in enumerate(dee_example.sentences):
            ...
            if sent_idx in dee_example.sent_idx2srange_mspan_mtype_tuples:
                # 返回每个句子拥有的mention，以及mention对应的位置和角色
        ...
        span_dranges_list = []
        mspan2span_idx = {}
        for mspan in dee_example.ann_valid_mspans:  # 文中所有无重复的实体名称
            if mspan in mspan2span_idx:
                continue

            # mspan 在文中出现的所有位置
            raw_dranges = dee_example.ann_mspan2dranges[mspan]
            char_base_s = 1 if self.include_cls else 0  # 在句首是否加 [CLS] 标志位
            char_max_end = self.max_sent_len - 1 if self.include_sep else self.max_sent_len  # 在句尾是否加 [SEP] 标志位
            span_dranges = []
            for sent_idx, char_s, char_e in raw_dranges:
                # 对于超出句子长度的 span 需要截断处理
            ...
            mspan2span_idx[mspan] = len(span_token_ids_list)  # mention的ID取决于之前mention的累加长度
            span_token_ids_list.append(span_token_ids)  # mention转换为ID列表添加到列表里面
        ...
        for event_idx, event_type in enumerate(self.event_type_list):
            ...
            if event_type not in dee_example.event_type2event_objs:
                ...
                for event_obj in event_objs:
                    ...
                    if any_valid_flag:
                        event_arg_idxs_objs.append(tuple(event_arg_idxs))  # [(x,None, x, x), (x,x,x)]

                if event_arg_idxs_objs:
                    event_type_labels.append(1)
                    event_arg_idxs_objs_list.append(event_arg_idxs_objs)  # [[event_arg_obj_ids_list], None]
                else:
                    event_type_labels.append(0)
                    event_arg_idxs_objs_list.append(None)
