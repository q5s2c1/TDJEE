...
def get_doc_span_info_list(doc_token_types_list, doc_fea_list, use_gold_span=False):
    assert len(doc_token_types_list) == len(doc_fea_list)  # len = batch_size
    doc_span_info_list = []
    # doc_token_types_list 是 NER 模型预测的结果，[batch, sen_num, sen_len]
    # 遍历整个batch
    for doc_token_types, doc_fea in zip(doc_token_types_list, doc_fea_list):
        # 一篇文章有多个句子，句子中每个字符对应一个 token_type
        doc_token_type_mat = doc_token_types.tolist()  # doc_token_types 可能是 tensor
        ...
        # span_mention_range_list 一篇文章中每个span对应mention出现的位置信息，里面具体的值是mention的起始和终止位置对应一个span
        # mention_drange_list 每个mention出现的具体句子，以及句中的起止位置
        # mention_type_list 每个mention对应的类型
...
class Doc2EDAGModel(nn.Module):
    ...
    def get_doc_span_mention_emb(self, doc_token_emb, doc_span_info):
        if len(doc_span_info.mention_drange_list) == 0:
            ...
            for sent_idx, char_s, char_e in doc_span_info.mention_drange_list:
                ...
                # 不同位置出现的mention具有不同的 embedding
                mention_emb_list.append(mention_emb)
                # mention_sent_id_list.append(sent_id)
    ...
    def get_doc_span_sent_context(self, doc_token_emb, doc_sent_emb, doc_fea, doc_span_info):
        """
        获取一篇文章中所有span的向量表示，将上下文信息融入span中。
        :param doc_token_emb:
        :param doc_sent_emb:
        :param doc_fea:
        :param doc_span_info:
        :return:
        """
        # mention 出现次数并不唯一   将 doc_token_emb 转换为 doc_mention_emb
        ...
        if doc_mention_emb is None:
            ...
            if self.config.use_doc_enc:
                ...
                # mention后面的sentence embedding也融入了文章信息
    ...
    def get_event_cls_info(self, sent_context_emb, doc_fea, train_flag=True):
        ...
        for event_idx, event_label in enumerate(doc_fea.event_type_labels):
            ...
            # 给event_table 传递 sentence_context_emb，返回是否存在当前类型事件
    ...
    def get_loss_on_doc(self, doc_token_emb, doc_sent_emb, doc_fea, doc_span_info):
        """
        获取一篇文章对应的loss，每次传递的是一个Document的数据进来
        :param doc_token_emb: 文章中所有token的embedding
        :param doc_sent_emb: 文章中所有句子的embedding
        :param doc_fea:
        :param doc_span_info:
        :return:
        """
        # 一个span表示一个独一无二的mention表示
        ...
        for event_idx, event_label in enumerate(doc_fea.event_type_labels):
            if event_label == 0:
                ...
                for field_idx in range(num_fields):
                    ...
                    # prepare label for candidate spans
                    # 每一个 span 都会分配一个角色标签，因为event_label=0，所以set为空，即所有field预测不为0都要计算到loss中
                    batch_span_label = get_batch_span_label(
                ...
                # 当event_label不为0时，需要和真实的DAG进行评估以计算loss
        ...
        # 最终的loss是事件类型分类和事件角色分类任务的loss之和
    ...
    def adjust_token_label(self, doc_token_labels_list):
        if self.config.use_token_role:  # do not use detailed token
            ...
            for doc_token_labels in doc_token_labels_list:
                ...
                entity_inside_mask = (doc_token_labels != 0) & (doc_token_labels % 2 == 0)  # doc_token_labels = 2
    ...
    def get_local_context_info(self, doc_batch_dict, train_flag=False, use_gold_span=False):
        ...
        valid_sent_num_list = doc_batch_dict['valid_sent_num']  # 每个batch对应的每篇文章中句子个数
    ...
    def forward(self, doc_batch_dict, doc_features,
        ...
        # 训练阶段使用了 scheduled sampling 策略进行优化
        ...
        # doc_fea_list [batch, token_id_list]
...
class AttentiveReducer(nn.Module):
    def __init__(self, hidden_size, dropout=0.1):
        ...
        # self.att_norm = math.sqrt(self.hidden_size)  # 768 无法开根号

        self.fc = nn.Linear(hidden_size, 1, bias=False)  # 输入: hidden_size   输出: 1
        self.att = None
    ...
    def forward(self, batch_token_emb, masks=None, keepdim=False):
        ...
        query = self.fc.weight  # query 为什么选 weight ？？？
        if masks is None:
            att_mask = None
        else:
...
class SentencePosEncoder(nn.Module):
    def __init__(self, hidden_size, max_sent_num=100, dropout=0.1):
        ...
        self.embedding = nn.Embedding(max_sent_num, hidden_size)  # [max_sent_num, hidden_size]
        self.layer_norm = transformer.LayerNorm(hidden_size)
        self.dropout = nn.Dropout(dropout)
...
class EventTable(nn.Module):
    def __init__(self, event_type, field_types, hidden_size):
        ...
        # Parameter函数可以理解为 类型转换函数
        # 将一个不可训练的类型Tensor转换为可以训练的类型parameter，并将这个parameter绑定到这个module里面
        # 经过类型转换后 self.event_query 变成模型的一部分
        # 使用这个函数的目的是为了让某些变量在学习过程中不断的修改其值以达到最优化目的
...
def get_span_mention_info(span_dranges_list, doc_token_type_list):
    ...
    for span_dranges in span_dranges_list:
        ...
        span_mention_range_list.append((ment_idx_s, ment_idx_e))  # 一个mention的起点和终点
...
def extract_doc_valid_span_info(doc_token_type_mat, doc_fea):
    ...
    for sent_idx in range(valid_sent_num):
        seq_token_id_list = doc_token_id_mat[sent_idx]  # 对应行号的句子中每个token的id
        seq_token_mask_list = doc_token_mask_mat[sent_idx]
        seq_token_type_list = doc_token_type_mat[sent_idx]  # if use_gold_span=false, 这里就是采用NER模型预测的token label
        ...
        while char_s < seq_len:
            ...
            entity_idx = seq_token_type_list[char_s]  # entity_id 是token的label: [O:0, B-xx:1, I-xx:2,...]

            if entity_idx % 2 == 1:  # 奇数情况下，是 B- 开头的标签
                char_e = char_s + 1
...
class DCFEEModel(nn.Module):
    ...
    def get_batch_sent_emb(self, ner_token_emb, ner_token_masks, valid_sent_num_list):
        """
        计算sentence embedding
        :param ner_token_emb: 整个batch的embedding
        :param ner_token_masks: token 对应的mask
        :param valid_sent_num_list:
        :return:
        """
        total_sent_emb = self.doc_token_reducer(ner_token_emb, masks=ner_token_masks)
    ...
    def get_doc_span_sent_context(self, doc_token_emb, doc_sent_emb, doc_span_info):
        """
        获取一篇文章中所有span和sentence的向量表示，并将上下文信息融入span和sentence中
        :param doc_token_emb:
        :param doc_sent_emb:
        :param doc_span_info:
        :return:
        """
        doc_sent_context = self.doc_context_encoder(doc_sent_emb.unsqueeze(0), None).squeeze(0)
        return None, doc_sent_context

    def get_loss_on_doc(self, doc_token_emb, doc_sent_emb, doc_fea, doc_span_info):
        """
        计算一篇文章内的所有类型的事件损失之和
        :param doc_token_emb: 传入该篇文章中句子中的 token 向量表示
        :param doc_sent_emb: 传入该篇文章所有句子的向量表示
        :param doc_fea: 文章其他特征向量
        :param doc_span_info:
        :return:
        """
        # 将篇章信息和span特征融入到句子向量中
        span_context_list, doc_sent_context = self.get_doc_span_sent_context(
            doc_token_emb, doc_sent_emb, doc_span_info,
        )

        assert doc_sent_context.size() == doc_sent_emb.size()
        ...
        for event_idx, event_cls in enumerate(self.event_cls_layers):
            doc_sent_logits = event_cls(doc_sent_context)
            doc_sent_labels = doc_sent_label_mat[:, event_idx]
    ...
    def get_mix_loss(self, doc_sent_loss_list, doc_event_loss_list, doc_span_info_list):
        ...
        lambda_1 = self.config.loss_lambda
        lambda_2 = 1 - lambda_1
        doc_ner_loss_list = []  # 一个batch中每篇文章的ner loss
        for doc_sent_loss, doc_span_info in zip(doc_sent_loss_list, doc_span_info_list):
            # doc_sent_loss: Size([num_valid_sents])
            sent_loss_scaling = doc_sent_loss.new_full(
                doc_sent_loss.size(), 1, requires_grad=False
            )
            # missed_sent_idx_list 表示该句子实际含有span，但没有预测出来
    ...
    def get_local_context_info(self, doc_batch_dict, train_flag=False, use_gold_span=False):
        ...
        doc_token_ids_list = doc_batch_dict['doc_token_ids']
        doc_token_masks_list = doc_batch_dict['doc_token_masks']
        valid_sent_num_list = doc_batch_dict['valid_sent_num']  # batch中每篇文章中句子个数
        ...
        # get ner output
        # 将整个batch里面的每个Document的句子融合起来计算损失，最终得到的是整个batch的损失
        # ner_token_emb: [sentences, tokens, hidden_size]
        ner_token_emb, ner_loss, ner_token_preds = self.ner_model(
            ner_token_ids, ner_token_masks, label_ids=ner_token_labels,
            train_flag=train_flag, decode_flag=not use_gold_span,
        )
    ...
    def forward(self, doc_batch_dict, doc_features,
        ...
        if train_flag:
            ...
            for batch_idx, ex_idx in enumerate(ex_idx_list):
                # 一个batch一个batch的处理
                eval_results.append(
                    self.heuristic_decode_on_doc(  # 返回的结果会打包成元组
                        doc_token_emb_list[batch_idx],
                        doc_sent_emb_list[batch_idx],
                        doc_fea_list[batch_idx],
                        doc_span_info_list[batch_idx],
            ...
            # 有batch大小个item，每个item是一个元组，元组里面包含了模型的返回结果
            return eval_results
    ...
    def heuristic_decode_on_doc(self, doc_token_emb, doc_sent_emb, doc_fea, doc_span_info,
        ...
        span_token_tup_list = doc_span_info.span_token_tup_list  # [(span1_token_id, ...), (...), ...]
        span_mention_range_list = doc_span_info.span_mention_range_list  # [(mention_s, mention_e)]
        mention_drange_list = doc_span_info.mention_drange_list  # [mention_drange, ...]
        mention_type_list = doc_span_info.mention_type_list  # [mention_type, ....]
        # heuristic decoding strategies will work on these span candidates
        # 返回训练数据中 mention_type, span_id, field_id, mention_range等的对应关系，全部都是ID之间的相互映射
        # 存放的是哪种类型的mention对应哪种类型的field等等关系，多重字典映射关系
        event_idx2field_idx2span_token_tup2dranges = self.get_event_field_span_candidates(
            span_token_tup_list, span_mention_range_list, mention_drange_list,
            mention_type_list, event_idx2entity_idx2field_idx,
        )
        ...
        event_idx2key_sent_idx_list = []  # 存放预测的不同类型的核心事件句
        event_pred_list = []
        event_idx2obj_idx2field_idx2token_tup = []
        for event_idx, event_cls in enumerate(self.event_cls_layers):
            ...
            if len(key_sent_idx_list) == 0:
                ...
                for key_sent_idx in key_sent_idx_list:
                    if heuristic_type == support_heuristic_types[0]:  # DCFEE-O single event
                        field_idx2token_tup = get_one_key_sent_event(
                            key_sent_idx, num_fields, field_idx2span_token_tup2dranges
                        )
                        obj_idx2field_idx2token_tup.append(field_idx2token_tup)
                    elif heuristic_type == support_heuristic_types[1]:  # DCFEE-M multi event
    ...
    def get_event_field_span_candidates(self, span_token_tup_list, span_mention_range_list,
        ...
        :param span_token_tup_list: 一篇文章里面所有的 span token idx
        :param span_mention_range_list: span 对应的 mentions
        :param mention_drange_list: mention 在文章中句子的具体位置
        :param mention_type_list: 预测的 mention 类型，该值为 label ID
        :param event_idx2entity_idx2field_idx: 双重映射字典，{event_id->{entity_type_id->field_id} }
        :return:
        """
        # get mention idx -> span idx
        # [span_1, span_1, span_2, span_2, span_2, ...]
        mention_span_idx_list = []
        for span_idx, (ment_idx_s, ment_idx_e) in enumerate(span_mention_range_list):
            mention_span_idx_list.extend([span_idx] * (ment_idx_e - ment_idx_s))
        assert len(mention_span_idx_list) == len(mention_drange_list)
        ...
        for event_idx, (event_type, field_types) in enumerate(self.event_type_fields_pairs):
            ...
            for ment_idx, (ment_drange, ment_entity_idx) in enumerate(zip(mention_drange_list, mention_type_list)):
                ...
                # mention_type -> field_type
                # 当前 mention_type 对应的事件 field 的 idx
                ment_field_idx = gold_entity_idx2field_idx[ment_entity_idx]
                if ment_field_idx is None:
                    continue
                ...
                # because it is dict, so all modifications to the key will take effect in raw dict
                # 当前 filed_idx 对应的 span_token_tuple, 以及当前的 span_token_tuple 对应的 mention_ranges
                # 也就是说有多个 mention 对应一个 span
                cur_span_token_tup2dranges = field_idx2span_token_tup2dranges[ment_field_idx]
                if span_token_tup not in cur_span_token_tup2dranges:
                    cur_span_token_tup2dranges[span_token_tup] = []
                cur_span_token_tup2dranges[span_token_tup].append(ment_drange)  # 字典列表，当前 span 对应的 mentions
